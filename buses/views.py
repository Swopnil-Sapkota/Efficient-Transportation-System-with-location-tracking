    ...
    if request.method == 'POST':
        ...
        if bus:
            ...
            if bus.remaining_seats > no_of_seats or bus.remaining_seats == int(no_of_seats) :
                ...
                if BookedSeats.objects.filter(bus=bus).exists():
                  bs=BookedSeats.objects.filter(bus=bus).first()
            
                  bs.seats=booked_seats 
                  bs.bus=bus
                  bs.save()
...
def bus_report(request):
    ...
    if request.method == 'POST':
        ...
        if search_results:
            ...
            buses=Bus.objects.all()
            return render(request,'buses/bus_report.html',{"error":"No reports to display",'buses':buses})
    else:
        buses=Bus.objects.all()
...
class BusCreateView(CreateView):
    model = Bus  
    fields=['name','source','destination','no_of_seats',
